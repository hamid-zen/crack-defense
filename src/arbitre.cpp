...
void  arbitre::updateSecondPlayer(t_action x )
{
    ...
    if (delays.cells_switch1 and delays.cells_switch2)
    {
        ...
        {
            _joueur2->setCellDy(*delays.cells_switch1, 8);
            _joueur2->setCellDy(*delays.cells_switch2, -8);
            if (_joueur2->cellDy(*delays.cells_switch1) >= 64 or _joueur2->cellDy(*delays.cells_switch2) <= -64)
            {
                _joueur2->switch_cells_position(*delays.cells_switch1, *delays.cells_switch2);
                _joueur2->resetCellDelta(*delays.cells_switch1);
                _joueur2->resetCellDelta(*delays.cells_switch2);
        ...
        {
            _joueur2->setCellDx(*delays.cells_switch1, 8);
            _joueur2->setCellDx(*delays.cells_switch2, -8);
            if (_joueur2->cellDx(*delays.cells_switch1) >= 64 or _joueur2->cellDx(*delays.cells_switch2) <= -64)
            {

                _joueur2->resetCellDelta(*delays.cells_switch1);
                _joueur2->resetCellDelta(*delays.cells_switch2);
                _joueur2->switch_cells_position(*delays.cells_switch1, *delays.cells_switch2);
                if (_joueur2->one_case_empty(*delays.cells_switch1, *delays.cells_switch2))
                { // si une deux cases etaient vide
                    _joueur2->slideColumn(delays.cells_switch1->x(), delays.cells_slide);
                    _joueur2->slideColumn(delays.cells_switch2->x(), delays.cells_slide);
                }
                delete delays.cells_switch1;
                delete delays.cells_switch2;
                delays.cells_switch1 = nullptr;
    ...
    if (x != t_action::nothing)
    {
        ...
        case t_action::exchange:
        {
            if (!_joueur2->target_cells_empty() and delays.cells_switch1 == nullptr and delays.cells_switch2 == nullptr and !(_joueur2->target_verticale() and _joueur2->one_case_empty())and !(_joueur2->is_garbage(_joueur2->getcell1target()) || _joueur2->is_garbage(_joueur2->getcell2target())))
            {

                delays.cells_switch1 = new position(_joueur2->getcell1target().x(), _joueur2->getcell1target().y());
                delays.cells_switch2 = new position(_joueur2->getcell2target().x(), _joueur2->getcell2target().y());
            }
        ...
        case t_action::generate_malus:
        {
            // On genere un malus
            // rajouter la touche pour envoyer un garbage a l'autre
            _joueur2->add_garbage(delays.cells_slide);
            break;
        }
        default: break;
        }
    ...
    {  if(delays.last_frame_alignment==-1)//pas encore initialisé
       ...
       //  _nb_frame++; // on incremente le nombre de frame
        ...
        if(delays.last_garbage>0){
            _joueur2->transform_malus_to_cell(v,delays.cells_slide);
            //ajouter que les cases glissent
        }
    ...
    if (delays.angle >= 360 || delays.scale <= 0) {
        delays.angle = 0;
        delays.scale = 1;
        _joueur2->inc_score(getDelays().cells_align.size());
        for (std::size_t i(0); i < v.size(); i++)
        {
            auto col(v[i].x());
            _joueur2->delete_cell(v[i]);
            _joueur2->slideColumn(col, delays.cells_slide);
        }
    }
