...
/*position game::malus_adjacent(position const & p) const{
    if(p.x()<_grid.max_width()-1 && _grid.estMalus(position (p.x()+1,p.y()))){
        return position (p.x()+1,p.y());
        _grid.
    }
    else if(p.x()>=1 && _grid.estMalus(position (p.x()-1,p.y()))){
        return position (p.x()-1,p.y());
    }
    else if(p.y()<_grid.max_height()-1 && _grid.estMalus(position (p.x(),p.y()+1))){
         return position (p.x(),p.y()+1);
    }
    if(p.y()>=1 && _grid.estMalus(position (p.x(),p.y()-1))){
        return position (p.x(),p.y()-1);
    }
