...
void interface::chat()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(e))
        {
            ...
            else if (e.type == sf::Event::KeyPressed && e.key.code == sf::Keyboard::Key::Enter)
            { // on click sur entrée
                _arbitre->send_string(message_input);
                if (!disconnected)
                {
            ...
            else if (e.type == sf::Event::KeyPressed && e.key.code == sf::Keyboard::Key::BackSpace)
            { // effacer la derniere lettre du message
        ...
        for (auto i(0); i < 9 && messages_focus + i < messages.size(); i++)
        { // on print que 9 messages (a cause de la hauteur de la fenetre)
...
void interface::play()
{
    ...
    std::vector<combo *> combosp1, combosp2;
    ...
    while (_window.isOpen() && (_arbitre->getDelays(true).activated || (_arbitre->jeu_duo() && _arbitre->getDelays(false).activated)))
    {
        ...
        auto it = combosp1.begin();
        while (it != combosp1.end())
        {
            if ((*it)->vitesse < 0)
            {
                delete *it;
                it = combosp1.erase(it);
            }
            else
            {
        ...
        auto it2 = combosp2.begin();
        while (it2 != combosp2.end())
        {
            if ((*it2)->vitesse < 0)
            {
                delete *it2;
...
void interface::pause_screen()
{
    ...
    while (_window.isOpen())
    {
        ...
        if (_arbitre->jeu_res())
        { // on check si l'autre joueur a repris le jeu
            t_action remote_player_action(t_action::nothing);
...
void interface::load_textures()
{
    ...
    _textures[t_textures_to_index(t_textures::P)].loadFromFile("../textures/keycaps/P.png");
...
void interface::menu_lan(bool disconnected)
{
    ...
    while (_window.isOpen())
    {
        ...
        if (tryconnect)
        {
            if (clock_for_circle.getElapsedTime().asSeconds() >= 1)
            { // plus d'une seconde est passé donc on change de texture pour le cercle
        ...
        { // si connecté on met le check_mark, un countdown et on affiche le boutton play
            ...
            if (clock_since_connection.getElapsedTime().asSeconds() >= 4)
            { // on attend 2 secondes avant de jouer // TODO: modifier
                play();
            }
        }
        ...
        if (disconnected && !_arbitre->connected())
        { // afichage de deconnexion si deconnecté
            if (static_cast<t_number>(clock_for_disconnect.getElapsedTime().asMilliseconds()) % 500 <= 250)
